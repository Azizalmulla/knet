...
export default function QRCard({ targetPath = '/start', fallbackImageSrc }: { targetPath?: string; fallbackImageSrc?: string }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-border shadow-xl bg-card p-4 flex items-center justify-center">
      {dataUrl ? (
        <img src={dataUrl} alt="Scan QR to begin" className="w-full h-auto" />)
        : (
        <div className="w-64 h-64 bg-muted animate-pulse rounded-xl" />
      )}
    </div>
  );
}
